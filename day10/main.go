...
func (b *bot) checkValues() {
	// fmt.Printf("Bot %d comparing values %d and %d\n", b.id, b.lowValue, b.highValue)
	if b.lowValue > b.highValue {
		panic("Values are wrong way round")
	}
	if b.lowValue == 17 && b.highValue == 61 {
		fmt.Printf("Finished! Bot is %d\n", b.id)
		// os.Exit(1)
	}
}

func getBot(id int) (b *bot) {
	var ok bool
	if b, ok = bots[id]; !ok {
		b = &bot{id: id}
		bots[id] = b
...
func (b *bot) take(v value) {
	// fmt.Printf("Bot %d receives %d\n", b.id, v)
	if v < b.highValue {
		b.lowValue = v
	} else {
...
func (o *output) take(v value) {
	if o.value == 0 {
		o.value = v
	}

	if outputs[0].value > 0 && outputs[1].value > 0 && outputs[2].value > 0 {
		fmt.Printf("Multiplied value is %d\n", outputs[0].value*outputs[1].value*outputs[2].value)
		os.Exit(1)
	}
}
...
func (b *bot) give() {
	b.checkValues()
	instruction := strings.Fields(b.instruction)
	...
	if instruction[5] == "output" {
		fmt.Printf("Bot %d gives %d to output %s\n", b.id, b.lowValue, instruction[6])
		destOutput, _ := strconv.Atoi(instruction[6])
		if destOutput < 3 {
			outputs[destOutput].take(b.lowValue)
		}
	} else {
		destBot, _ := strconv.Atoi(instruction[6])
		db := getBot(destBot)
		db.take(b.lowValue)
	}

	if instruction[10] == "output" {
		fmt.Printf("Bot %d gives %d to output %s\n", b.id, b.highValue, instruction[11])
		destOutput, _ := strconv.Atoi(instruction[6])
		if destOutput < 3 {
			outputs[destOutput].take(b.highValue)
		}
	} else {
		destBot, _ := strconv.Atoi(instruction[11])
		db := getBot(destBot)
		db.take(b.highValue)
	}
}

func (b *bot) giveIfReady() {
	if b.lowValue > 0 && b.highValue > 0 && b.instruction != "" {
		b.give()
		// } else {
		// 	fmt.Printf("Bot %d not ready: %d, %d,'%s'\n", b.id, b.lowValue, b.highValue, b.instruction)
	}
}
...
func main() {
	...
	for _, i := range strings.Split(input, "\n") {
		...
		if instruction[0] == "bot" {
			botID, _ := strconv.Atoi(instruction[1])
			if botID > botCount {
				botCount = botID
			}
		}
	...
	fmt.Printf("Highest bot ID %d\n", botCount)
	bots = make(map[int]*bot, botCount)
	outputs = make(map[int]*output, 3)
	outputs[0] = &output{}
	outputs[1] = &output{}
	outputs[2] = &output{}
	...
	for _, i := range strings.Split(input, "\n") {
		...
		case "value":
			botID, _ := strconv.Atoi(instruction[5])
			v, _ := strconv.Atoi(instruction[1])
			if v == 0 {
				panic("Need to cope with zero value")
			}
...
var input = `bot 135 gives low to bot 27 and high to bot 166
