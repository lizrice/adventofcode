...
func v1() {
	...
	output := make([]byte, 50*len(input))
	i := 0
	o := 0
	for i < len(input) {
		if input[i] == '(' {
			markerX := strings.Index(input[i:], "x")
			markerEnd := strings.Index(input[i:], ")")
			if markerEnd == -1 || markerX == -1 {
				panic("No x or close to marker")
			}

			repeatLength, _ := strconv.Atoi(input[i+1 : i+markerX])
			repeatCount, _ := strconv.Atoi(input[i+markerX+1 : i+markerEnd])
			// fmt.Printf("%d x %d\n", repeatLength, repeatCount)
			for ; repeatCount > 0; repeatCount-- {
				for r := 0; r < repeatLength; r++ {
					output[o] = input[i+markerEnd+1+r]
					o++
				}
			}
			i += markerEnd + repeatLength
		} else {
			output[o] = input[i]
			o++
		}
...
func findRepeat(b string) (int, int, int) {
	markerX := strings.Index(b, "x")
	markerEnd := strings.Index(b, ")")
	if markerEnd == -1 || markerX == -1 {
		panic("No x or close to marker")
	}

	repeatLength, _ := strconv.Atoi(b[1:markerX])
	repeatCount, _ := strconv.Atoi(b[markerX+1 : markerEnd])
	// fmt.Printf("Repeat %d chars %d times\n", repeatLength, repeatCount)
	return repeatLength, repeatCount, markerEnd + 1
}

func decompress(b string) int {
	length := 0
	for i := 0; i < len(b); i++ {
		if b[i] == '(' {
			repeatLength, repeatCount, markerLength := findRepeat(b[i:])
			i += markerLength
			length += repeatCount * decompress(b[i:i+repeatLength])
			i += repeatLength - 1
		} else {
			// Not inside a repeat - this is a normal character
			// fmt.Printf("Single char %c\n", b[i])
			length++
		}
	}

	// fmt.Printf("Decompressed %s length %d\n", string(b), length)
