...
// Return true if this string contains an abba
...
func babmatch(s, t string) bool {
	if len(t) != 3 || len(s) != 3 {
		fmt.Printf("%s %s", s, t)
		panic("unexpected length of bab")
	}

	if t[0] != t[2] || s[0] != s[2] {
		panic("strings aren't aba")
	}
	return t[0] == s[1] && t[1] == s[0]
}

func tls(ip string) bool {
	outer := true
	candidate := false
	sub := ""
	for {
		split := strings.IndexAny(ip, "[]")
		if split == -1 {
			sub = ip
		} else {
			sub = ip[:split]
		...
		if abba(sub) {
			if !outer {
				// fmt.Printf("inner %s is abba\n", sub)
				return false
			}
			// fmt.Printf("outer %s is abba\n", sub)
			candidate = true
			// } else {
			// 	fmt.Printf("%s is not abba - outer? %t\n", sub, outer)
		}
...
func ssl(ip string) bool {
	...
	for split != -1 {
		...
		if outer {
			outerAba = append(outerAba, aba(sub)...)
		} else {
			innerAba = append(innerAba, aba(sub)...)
		}
	...
	for _, aba := range outerAba {
		for _, bab := range innerAba {
			if babmatch(aba, bab) {
				return true
			}
		}
...
func main() {
	...
	// for _, s := range []string{"abba", "mnop", "aaaa", "ioxxoj"} {
	// 	fmt.Printf("%s - %t\n", s, abba(s))
	// }

	// fmt.Println("TLS:")
	// for _, s := range []string{"abba[mnop]qrst", "abcd[bddb]xyyx", "aaaa[qwer]tyui", "ioxxoj[asdfgh]zxcvbn", "ajciwh[jchsoaud]jsicjs[jcdichs]abba"} {
	// 	fmt.Printf("%s - %t\n", s, tls(s))
	// }

	fmt.Println("SSL:")
	for _, s := range []string{"aba[bab]xyz", "xyx[xyx]xyx", "aaa[kek]eke", "zazbz[bzb]cdb"} {
		fmt.Printf("%s - %t\n", s, ssl(s))
	}
...
var input = `xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
