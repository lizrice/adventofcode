...
var visitedBefore map[thing]struct{}
...
// Data from A could fit on B
func (s state) viable(A, B coord) bool {
	if A.x == B.x && A.y == B.y {
		// Same node
		return false
	}

	v := s.nodes[A].used > 0 && s.nodes[A].used <= s.nodes[B].avail
	return v
}

func (s state) viableMoves() (moves []move) {
	for A := range s.nodes {
		// North
		if A.y > 0 {
			B := coord{x: A.x, y: A.y - 1}
			if s.viable(A, B) {
...
func (s state) makeMove(m move) state {
	var newState state
	newState.goal.x = s.goal.x
	newState.goal.y = s.goal.y
	newState.empty = make([]coord, len(s.empty))
	for i, e := range s.empty {
		newState.empty[i] = e
	}

	newState.nodes = make(map[coord]node, len(s.nodes))
	for k, v := range s.nodes {
		newState.nodes[k] = v
	}

	if m.A.x == s.goal.x && m.A.y == s.goal.y {
		// fmt.Printf("Goal moves to %v \n", m.B)
		newState.goal = m.B
	}

	for i, e := range newState.empty {
		if m.B.x == e.x && m.B.y == e.y {
			newState.empty[i] = m.A
			// fmt.Printf("Empty moves to %v \n", m.A)
		}
	}

	newState.move(m.A, m.B)
	// fmt.Printf("After move state %v\n", newState)
	// fmt.Printf("And original state %v\n", s)
	return newState
}
...
func visited(s state) bool {
	t := thing{
		goal:  s.goal,
		empty: s.empty[0],
	}

	_, ok := visitedBefore[t]
	if !ok {
		visitedBefore[t] = struct{}{}
	}

	return ok
}

// func match(s state, ts trackState) bool {
// 	// fmt.Printf("Comparing %v and %v\n", s, ts)
// 	if len(ts.empty) > 1 {
// 		panic("More than one empty node")
// 	}
// 	return s.goal.x == ts.goal.x && s.goal.y == ts.goal.y &&
// }

func finished(ss []state) bool {
	for _, s := range ss {
		if s.goal.x == 0 && s.goal.y == 0 {
...
func main() {
	...
	// To start with goal data is in the top right
	s := state{goal: coord{x: gridSize - 1, y: 0}}
	// ts := trackState{goal: coord{x: gridSize - 1, y: 0}}
	s.nodes = make(map[coord]node, len(lines)-2)
	...
	for i, line := range lines {
		...
		coords := strings.Split(words[0], "-")
		x, _ := strconv.Atoi(strings.TrimPrefix(coords[1], "x"))
		y, _ := strconv.Atoi(strings.TrimPrefix(coords[2], "y"))
		...
		c := coord{x: x, y: y}
		s.nodes[c] = node{size: size, used: used, avail: avail}
	...
	fmt.Printf("%d nodes\n", len(s.nodes))
	viableCount := 0
	emptyNodes := make(map[coord]bool, 1)
	for aa := range s.nodes {
		for bb := range s.nodes {
			if s.viable(aa, bb) {
				viableCount++
				if _, ok := emptyNodes[bb]; !ok {
					fmt.Printf("node at %v is empty\n", bb)
					emptyNodes[bb] = true
					s.empty = append(s.empty, bb)
					// ts.empty = append(ts.empty, bb)
				}
			}
		}
	}

	fmt.Println(viableCount)

	states := []state{s}
	fmt.Printf("Initial state: %v\n", s)
	visitedBefore = make(map[thing]struct{}, gridSize*gridSize*2)
	t := thing{
		goal:  s.goal,
		empty: s.empty[0],
	}
