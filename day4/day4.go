...
func (c charMapList) Len() int { return len(c) }
func (c charMapList) Less(i, j int) bool {
	if c[i].count == c[j].count {
		return c[i].character < c[j].character
	}
	return c[i].count > c[j].count
}
func (c charMapList) Swap(i, j int) { c[i], c[j] = c[j], c[i] }

func decode(room string, sum int) string {
	decoded := make([]byte, len(room))
	cipher := sum % 26
	var shifted int
	for i, c := range room {
		switch c {
		case '-':
			shifted = int(' ')
		default:
			shifted = int(c) + cipher
			if shifted > int('z') {
				shifted -= 26
			}
		}
		decoded[i] = byte(shifted)
	}
...
func main() {
	...
	for _, room := range rooms {
		...
		checksum := strings.TrimSuffix(split[1], "]")
		roomSplit := strings.Split(split[0], "-")
		roomNum, _ := strconv.Atoi(roomSplit[len(roomSplit)-1])
		roomletters := strings.TrimSuffix(split[0], roomSplit[len(roomSplit)-1])
		roomletters = strings.Replace(roomletters, "-", "", -1)
		// fmt.Println(checksum)
		// fmt.Println(roomNum)
		// fmt.Println(roomletters)
		// fmt.Println(re.FindAllString(split[0], -1))

		m := make(map[rune]int)
		for _, c := range roomletters {
			_, ok := m[c]
			if ok {
				continue
			}

			m[c] = strings.Count(roomletters, string(c))
		}
		...
		cm := make(charMapList, len(m))
		i := 0
		for k, v := range m {
			cm[i] = charMap{character: k, count: v}
			i++
		}
		...
		if match {
			roomSum += roomNum
			decoded := decode(split[0], roomNum)
			if strings.Contains(decoded, "orth") {
				fmt.Println(decoded)
				fmt.Println(roomNum)
			}
		}
...
var input = `aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
